...
CONFHOST=avatar.yourdomain.net
...
trap exit 2
#rpm -q $line --quiet
rpm -V $line --nomtime --nouser --nogroup --nomode
if [ $? -eq 1 ]
then
...
if [ $verbose ]
then
debug "rpm -ev $p --nodeps --justdb"
fi
rpm -ev $p --nodeps --justdb
if [ $? -eq 0 ]
then
...
if [ -z "$colo" ]; then
	$verbose && debug "Can not determine coloname, use default your_yum_server."
	colo='corp'
fi
if [ "$colo" = "corp" ]; then
	YUMHOST="your_yum_server"
fi
if [ -z "$YUMHOST" ]; then
	YUMHOST="http://yum.$colo"
else
...
function version () {
	...
	walle_say "$MYNAME"
	echo $VER
	echo "Visit http://code.google.com/p/taobao-avatar/ for more infomation"
        printf "
---Taobao Packages---
------======\\ \\------
-----(O)(O)=--,,-----
...
function summary() {
	...
	cat $cust_rpms_log
	echo ""
	warning "# CUST RPMS NEED TO REMOVE:"
...
function locate_rpm_url(){
	...
	if [ $verbose ]
	then
		debug "http://your_yum_server/cgi-bin/rpmfind?name="$name".rpm";
	fi
	rpm_location=`curl -s 'http://your_yum_server/cgi-bin/rpmfind?name='$name'.rpm'`;
	rpm_location=`echo $rpm_location | sed -e 's#http://your_yum_server#'$YUMHOST'#g'`
	if [ ! -n "$rpm_location" ]
	then
...
function parse_rpm_name (){
if [[ "$a" =~ "(\S+)-(\S+)-(.*)" ]]
then
local name=${BASH_REMATCH[1]}
local version=${BASH_REMATCH[2]}
local rest_string=${BASH_REMATCH[3]}
local arch=${rest_string##*.}
fi
}
